...
export default function ReportDetailPage({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        reportsService.getById(params.id).then(s => s && setSession(s))
    }, [params.id])
            ...
            <div className="flex items-center gap-4 border-b pb-4">
                ...
                <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                        <Badge variant={session.status === 'flagged' ? 'destructive' : 'outline'}>{session.status}</Badge>
                        <span className="text-sm text-muted-foreground">{new Date(session.startedAt).toLocaleString()}</span>
                    </div>
                </div>
                ...
                <Card className="md:col-span-2">
                    ...
                    <CardContent>
                        <div className="relative border-l border-muted ml-4 space-y-8 pb-4">
                            {session.events.length > 0 ? session.events.map((event, i) => (
                                <div key={event.id} className="relative pl-6">
                                    ...
                                    <div className="flex flex-col gap-1">
                                        <span className="text-sm font-semibold capitalize">{event.type.replace(/_/g, ' ')}</span>
                                        <span className="text-xs text-muted-foreground">{new Date(event.timestamp).toLocaleTimeString()}</span>
                                        <Badge className="w-fit" variant={event.severity === 'high' ? 'destructive' : 'secondary'}>{event.severity}</Badge>
                                    </div>
                                </div>
                            )) : (
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        <div className="flex justify-between items-center">
                            ...
                            <span className="text-2xl font-bold">{session.flagsCount}</span>
                        </div>
                        <div className="flex justify-between items-center">
                            <span className="text-sm font-medium">Risk Level</span>
                            <span className={`text-lg font-bold ${session.flagsCount > 5 ? 'text-destructive' : 'text-green-600'}`}>
                                {session.flagsCount > 5 ? 'High' : 'Low'}
                            </span>
                        </div>
                    </CardContent>
                </Card>
